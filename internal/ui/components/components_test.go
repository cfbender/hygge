...
// ── Typing animation (fade-in) tests ─────────────────────────────────────────

// TestApplyTypingFade_NonStreamingPassthrough verifies that a completed
// (non-streaming) message is returned unchanged — no faint style applied.
func TestApplyTypingFade_NonStreamingPassthrough(t *testing.T) {
	t.Parallel()
	body := "line one\nline two\nline three"
	got := applyTypingFade(body, false, 0)
	if got != body {
		t.Errorf("non-streaming body should be unchanged; got %q, want %q", got, body)
	}
}

// TestApplyTypingFade_AllRevealed verifies that a body where all characters are
// already revealed (revealedChars >= total) is returned unchanged.
func TestApplyTypingFade_AllRevealed(t *testing.T) {
	t.Parallel()
	body := "line one\nline two\nline three"
	got := applyTypingFade(body, true, 100)
	if got != body {
		t.Errorf("fully-revealed body should be unchanged; got %q, want %q", got, body)
	}
}

// TestApplyTypingFade_PartialReveal verifies that characters beyond revealedChars
// are wrapped in a faint style and that revealed characters are unchanged.
func TestApplyTypingFade_PartialReveal(t *testing.T) {
	t.Parallel()
	body := "revealed\nnew line"
	got := applyTypingFade(body, true, len([]rune("revealed")))
	if !strings.HasPrefix(got, "revealed") {
		t.Errorf("revealed prefix should be unchanged, got %q", got)
	}
	if !strings.Contains(got, "\x1b[2m") {
		t.Errorf("unrevealed text should contain faint ANSI sequence; got %q", got)
	}
	if stripANSI(got) != body {
		t.Errorf("plain text should be preserved; got %q, want %q", stripANSI(got), body)
	}
}

// TestApplyTypingFade_ZeroRevealedFadesAll verifies that with revealedChars=0
// all characters are faint during streaming.
func TestApplyTypingFade_ZeroRevealedFadesAll(t *testing.T) {
	t.Parallel()
	body := "line one\nline two"
	got := applyTypingFade(body, true, 0)
	plain := stripANSI(got)
	// Plain text should still be present.
	if !strings.Contains(plain, "line one") || !strings.Contains(plain, "line two") {
		t.Errorf("all characters should still be present; got %q", plain)
	}
	// Every line should have faint sequence.
	for _, line := range strings.Split(got, "\n") {
		if !strings.Contains(line, "\x1b[2m") {
			t.Errorf("all characters should be faint when revealedChars=0; line %q has no faint", line)
		}
	}
}

// TestAssistantBubble_CompletedMessageNoFade verifies that a completed
// (non-streaming) assistant bubble renders its body without any faint styling.
func TestAssistantBubble_CompletedMessageNoFade(t *testing.T) {
	t.Parallel()
	ml := MessageList{
		Width: 80,
		Theme: theme.ShellTheme(),
		Messages: []UIMessage{
			{
				Role:          RoleAssistant,
				Raw:           "Hello from the assistant",
				FinalMarkdown: "Hello from the assistant",
				IsStreaming:   false,
				RevealedChars: 0, // should be ignored for completed messages
			},
		},
	}
	out := ml.View()
	// Strip ANSI and verify content is present.
	plain := stripANSI(out)
	if !strings.Contains(plain, "Hello from the assistant") {
		t.Errorf("completed assistant bubble missing body text; got:\n%s", plain)
	}
	// No faint codes should appear in the body — completed messages render normally.
	// Check the line containing the body text.
	for _, line := range strings.Split(out, "\n") {
		if strings.Contains(stripANSI(line), "Hello from the assistant") {
			if strings.Contains(line, "\x1b[2m") {
				t.Errorf("completed message body should not be faint; line: %q", line)
			}
		}
	}
}

// TestAssistantBubble_StreamingMessageFadesNewChars verifies that a streaming
// assistant bubble renders unrevealed texts with faint styling.
func TestAssistantBubble_StreamingMessageFadesNewChars(t *testing.T) {
	t.Parallel()
	ml := MessageList{
		Width: 80,
		Theme: theme.ShellTheme(),
		Messages: []UIMessage{
			{
				Role:          RoleAssistant,
				Raw:           "revealed line\nnew faint chars",
				FinalMarkdown: "revealed line\nnew faint chars",
				IsStreaming:   true,
				RevealedChars: 1, // first text revealed, second faint
			},
		},
	}
	out := ml.View()
	plain := stripANSI(out)
	// Both lines must appear in the output.
	if !strings.Contains(plain, "revealed line") {
		t.Errorf("streaming bubble missing revealed line; got:\n%s", plain)
	}
	if !strings.Contains(plain, "new faint chars") {
		t.Errorf("streaming bubble missing faint line content; got:\n%s", plain)
	}
	// The faint line must have ANSI faint somewhere in the output.
	if !strings.Contains(out, "\x1b[2m") {
		t.Errorf("streaming bubble should apply faint to unrevealed texts; got:\n%s", out)
	}
}

// TestAssistantBubble_StreamingFullyRevealedNoFade verifies that when all
// characters are revealed during streaming, no faint is applied.
func TestAssistantBubble_StreamingFullyRevealedNoFade(t *testing.T) {
	t.Parallel()
	body := "line one\nline two"
	ml := MessageList{
		Width: 80,
		...
		Messages: []UIMessage{
			{
				...
				FinalMarkdown: body,
				IsStreaming:   true,
				RevealedChars: 100, // well beyond line count
			},
		},
	}
	out := ml.View()
	// Faint ANSI must not appear in the content lines.
	for _, line := range strings.Split(out, "\n") {
		if strings.Contains(stripANSI(line), "line one") || strings.Contains(stripANSI(line), "line two") {
			if strings.Contains(line, "\x1b[2m") {
				t.Errorf("fully-revealed streaming text should not be faint; line: %q", line)
			}
		}
	}
}
