...
func buildLatestUserEnvelope(userText string, memories []*session.Memory, contextWindow int64, usedTokens int64, pctUsed float64) string {
	...
	b.WriteString("    - You may propose memories for repeated or clear user-authored preferences/facts; save session task constraints autonomously only when obvious.\n")
	b.WriteString("    - Require explicit user confirmation before saving inferred project/global memories; never remember secrets, guesses, or transient task state.\n")
