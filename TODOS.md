...
- [ ] **Phase 6 — UI** — Theme/model-select/API-key dialogs; slash command system update.
  ...
  - [x] **Phase 6F — Model selection dialog** — `/model` opens an overlay-stack catalog browser with search, metadata rows, keyboard navigation, runtime model switching, and safe best-effort config persistence for successful selections.
  - [ ] **Phase 6F follow-ups** — Refresh the live runtime resolver/agent for already-running turns; add a settings status line that distinguishes persisted profile defaults from session-local overrides.
