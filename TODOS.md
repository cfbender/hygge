...
- [ ] **Phase 6 — UI** — Theme/model-select/API-key dialogs; slash command system update.
  ...
  - [x] **Phase 6E — Prompt file attachments** — Added `/attach <path>` and `/attachments clear` for one-shot local text/image prompt attachments, compact pending chips near the input, 64KB text limit, binary rejection, and send-path propagation through `session.Part` persistence/provider conversion.
  - [x] **Phase 6F — Model selection dialog** — `/model` opens an overlay-stack catalog browser with search, metadata rows, keyboard navigation, and current-session model switching. Safe config persistence/runtime rewire is deferred; slash `/model <provider>/<model>` remains a direct current-session switch.
  - [ ] **Phase 6F follow-ups** — Persist selected provider/model through the config/profile writer when a safe settings seam exists; refresh the live runtime resolver/agent for already-running turns; add a settings status line that distinguishes persisted profile defaults from session-local overrides.
  - [ ] **Phase 6G+ follow-ups** — API-key and theme dialogs on the overlay stack; richer command dialogs; durable todo persistence/hydration; richer attachment previews/removal shortcuts; optional backdrop rendering over preserved app content instead of full-screen modal replacement.
